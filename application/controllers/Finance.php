...
class Finance extends MY_Controller {
	...
	public function bgc()
	{
		...
		$data['addr'] = (!empty($setting)) ? $setting[0]['from_addr'] : '';
	...
	public function submitWithdraw()
	{
		...
		$wCode = uniqid();
        $wLink = site_url().'finance/withdraw/confirm?wCode='.utf8_encode(base64_encode($wCode)).'&wid='.$this->userInfo['user_id'].'&currency='.$request->coinType.'&from='.$request->fromAddr.'&to='.$request->toAddr.'&amount='.$request->amount.'&note=withdraw';
